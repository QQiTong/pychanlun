...
- 从gateway文件夹引入接口类文件，如from vnpy.vnpy.gateway.ctp import CtpGateway;
...
```
from vnpy.vnpy.gateway.tota import ToraGateway
main_engine.add_gateway(OesGateway)
```
...
- 服务端：运行vntrader时加载rpc_service模块
    ```
    from vnpy.vnpy.app.rpc_service import RpcService
    ```
