...
def calc(time_s, macd_s, diff_s, dea_s, bi_list, duan_s):
    ...
    for i in range(len(duan_s)):
        if duan_s[i] == -1:
            duan_start = pydash.find_last_index(duan_s[:i], lambda d: d == 1) - 1
            duan_end = i
        ...
        if duan_s[i] == 1:
            duan_start = pydash.find_last_index(duan_s[:i], lambda d: d == -1) - 1
            duan_end = i
            up_bi_list = pydash.filter_(bi_list, lambda bi: bi.direction == 1 and bi.end <= duan_end and bi.start >= duan_start)
            if len(up_bi_list) > 1:
