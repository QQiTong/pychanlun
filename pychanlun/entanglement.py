                ...
                if r >= 0:
                    ...
                    if ma20 is not None:
                        ...
                        if t in ma20 and e.top < ma20[t]:
                            above_ma20 = False
                    result['sell_zs_huila']['above_ma20'].append(above_ma20)
        if e.direction == -1:
            # 下跌中枢，找第一次的拉回
            leave = e.end
                ...
                if r >= 0:
                    ...
                    if ma20 is not None:
                        ...
                        if t in ma20 and e.bottom > ma20[t]:
                            above_ma20 = True
                    result['buy_zs_huila']['above_ma20'].append(above_ma20)
    return result
        ...
        if e.direction == -1:
            ...
            if r > 0:
                ...
                if ma20 is not None:
                    t = datetime.datetime.strptime(time_series[r], "%Y-%m-%d %H:%M")
                    t = t.replace(hour=0, minute=0).timestamp()
                    if t in ma20 and e.dd < ma20[t]:
                        above_ma20 = False
                result['sell_zs_tupo']['above_ma20'].append(above_ma20)
    return result
                    ...
                    if low_series[k] < low_series[d1]:
                        ...
                        if pydash.find_last_index(result['sell_five_v_reverse']['date'], lambda x: x == time_series[k]) == -1:
                            ...
                            if ma20 is not None:
                                t = datetime.datetime.strptime(time_series[k], "%Y-%m-%d %H:%M")
                                t = t.replace(hour=0, minute=0).timestamp()
                                if t in ma20 and low_series[d1] < ma20[t]:
                                    above_ma20 = False
                            result['sell_five_v_reverse']['above_ma20'].append(above_ma20)
                        break
                    if bi_series[k] == -1:
                        break
                    ...
                    if high_series[k] > high_series[g1]:
                        ...
                        if pydash.find_last_index(result['buy_five_v_reverse']['date'], lambda x: x == time_series[k]) == -1:
                            ...
                            if ma20 is not None:
                                t = datetime.datetime.strptime(time_series[k], "%Y-%m-%d %H:%M")
                                t = t.replace(hour=0, minute=0).timestamp()
                                if t in ma20 and high_series[g1] > ma20[t]:
                                    above_ma20 = False
                            result['buy_five_v_reverse']['above_ma20'].append(above_ma20)
                        break
                    if bi_series[k] == -1:
                        break
    return result
                        ...
                        if low_series[k] < resist_price:
                            ...
                            if pydash.find_last_index(result['sell_v_reverse']['date'],
                                ...
                                if ma20 is not None:
                                    t = datetime.datetime.strptime(time_series[k], "%Y-%m-%d %H:%M")
                                    t = t.replace(hour=0, minute=0).timestamp()
                                    if t in ma20 and resist_price < ma20[t]:
                                        above_ma20 = False
                                result['sell_v_reverse']['above_ma20'].append(above_ma20)
                            break
        if e.direction == -1:
            # 离开中枢后的第一段结束
            leave_end_index = -1
                        ...
                        if high_series[k] > resist_price:
                            ...
                            if pydash.find_last_index(result['buy_v_reverse']['date'],
                                ...
                                if ma20 is not None:
                                    ...
                                    if t in ma20 and resist_price > ma20[t]:
                                        above_ma20 = True
                                result['buy_v_reverse']['above_ma20'].append(above_ma20)
                            break
    return result
                ...
                for k in range(anchor + 1, len(time_series)):
                    ...
                    if low_series[k] < low_series[anchor]:
                        ...
                        if ma20 is not None:
                            t = datetime.datetime.strptime(time_series[k], "%Y-%m-%d %H:%M")
                            t = t.replace(hour=0, minute=0).timestamp()
                            if t in ma20 and low_series[anchor] < ma20[t]:
                                above_ma20 = False
                        result['sell_duan_break']['above_ma20'].append(above_ma20)
                        break
                ...
                for k in range(anchor + 1, len(time_series)):
                    ...
                    if high_series[k] > high_series[anchor]:
                        ...
                        if ma20 is not None:
                            t = datetime.datetime.strptime(time_series[k], "%Y-%m-%d %H:%M")
                            t = t.replace(hour=0, minute=0).timestamp()
                            if t in ma20 and high_series[anchor] > ma20[t]:
                                above_ma20 = True
                        result['buy_duan_break']['above_ma20'].append(above_ma20)
                        break
    return result
