...
def find_pivots(from_idx, to_idx, duan_series, high_series, low_series, direction):
    pivots = []
    if direction == -1:
        start = from_idx
        end = to_idx + 1
        ...
        while True:
            ...
            if g == -1:
                break
            g = d + g
            sequence.append({ 'start': d, 'end': g, 'low': low_series[d], 'high': high_series[g] })
            start = g
        # 至少有2个特征序列才可能出现中枢
        if len(sequence) >= 2:
        ...
        while True:
            ...
            if d == -1:
                break
            d = g + d
            sequence.append({ 'start': g, 'end': d, 'low': low_series[d], 'high': high_series[g] })
            start = d
        # 至少有2个特征序列才可能出现中枢
        if len(sequence) >= 2:
            pivot = { 'sequence_count': 0 }
